...
func doInit(server *protocolServer, req *request) {
	...
	if server.opts.EnableWritebackCache {
		kernelFlags |= input.Flags64() & CAP_WRITEBACK_CACHE
	} else {
		kernelFlags &= ^uint64(CAP_WRITEBACK_CACHE)
	}
	...
	*out = InitOut{
	...
	logger.Printf("fuse init writeback-cache requested=%t kernel_support=%t negotiated=%t",
		server.opts.EnableWritebackCache,
		input.Flags64()&CAP_WRITEBACK_CACHE != 0,
		kernelFlags&CAP_WRITEBACK_CACHE != 0,
	)
